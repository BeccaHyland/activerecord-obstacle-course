...
describe 'ActiveRecord Obstacle Course' do
  ...
  it 'finds orders by amount' do
    ...
    # or Order.where("amount=?", 500) <-- the question mark here is called a 'binding parameter'
  ...
  it 'finds orders for a user' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    user = User.find(3)
    orders_of_user_3 = Order.where(user: user)
    # ------------------------------------------------------------
  ...
  it 'returns the sorted names of items for a user order' do
    ...
    # end
    # ------------------------------------------------------------
    #we want all the items from user 3's third order
    #the where clause is sent aas a hash

    #get an array of the user's orders
    #get the third element out of the list
    #get the items from that order
    #get the names

    # ------------------ Using ActiveRecord ----------------------
    items_for_user_3_third_order =
    User.find(3).orders.third.items.pluck(:name)
    # ------------------------------------------------------------
